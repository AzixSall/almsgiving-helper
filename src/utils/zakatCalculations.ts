...
export const calculateNisabThreshold = (
  ...
  const goldNisab =
    PreciousMetalPrices[currency].goldPricePerGram *
    PreciousMetalPrices.nisabGoldWeight;
  const silverNisab =
    PreciousMetalPrices[currency].silverPricePerGram *
    PreciousMetalPrices.nisabSilverWeight;
...
export const calculateZakat = (
  ...
  const zakatPayable = eligibleForZakat
    ? netZakatableAmount * PreciousMetalPrices.zakatRate
    : 0;
 ...
 */
export const calculateZakatFitr = (
  values: ZakatFitrValues,
  currency: "USD" | "EUR" | "XOF" = "USD"
): ZakatFitrResults => {
  ...
  if (values.paymentMethod === "food") {
    ...
    totalAmount =
      values.familyMembers * PreciousMetalPrices[currency].fitrAmountPerPerson;
  }
